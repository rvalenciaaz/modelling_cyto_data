...
# ---------------------------------------------------------
# 6. 5-Fold Nested Cross-Validation using Best Hyperparameters
# ---------------------------------------------------------
...
for fold, (train_idx, val_idx) in enumerate(outer_cv.split(X_train_scaled), start=1):
    ...
    # Build new model instance with fixed best hyperparameters
    model, _, _ = create_vae_model(
        input_dim,
    ...
    # Train the model on the training split for this fold
    ...
    # Evaluate on the outer fold's validation set
    fold_loss = model.evaluate(X_val_fold, X_val_fold, batch_size=best_params["batch_size"], verbose=0)
...
# ---------------------------------------------------------
# 7. Train final VAE model using best hyperparameters (increase epochs)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# 8. Evaluate on the test set and extract latent space encoding
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# 9. Plot and save training history (loss curves)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# 10. Save final model weights, scaler, and logs
# ---------------------------------------------------------
