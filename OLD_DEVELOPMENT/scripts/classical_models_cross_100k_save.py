...
grid_rf = GridSearchCV(
    ...
    cv=3,                # grid search uses 3-fold CV
    scoring='accuracy',
...
cv_results_rf.to_csv("cv_results_rf.csv", index=False)
...
# 6.1 5-FOLD CROSS-VALIDATION FOR LOGISTIC REGRESSION
cv_scores_lr = cross_val_score(best_lr, X_train_scaled, y_train, cv=5, scoring='accuracy')
print("\n=== Logistic Regression 5-Fold Cross-Validation ===")
print(f"Fold Accuracies: {cv_scores_lr}")
print(f"Mean CV Accuracy: {cv_scores_lr.mean():.4f}")
print(f"CV Accuracy Std Dev: {cv_scores_lr.std():.4f}")
cv_results_lr = pd.DataFrame({
    "Fold": list(range(1,6)),
    "Accuracy": cv_scores_lr
})
cv_results_lr.to_csv("cv_results_lr.csv", index=False)
...
# 7.1 5-FOLD CROSS-VALIDATION FOR XGBOOST
cv_scores_xgb = cross_val_score(best_xgb, X_train_scaled, y_train, cv=5, scoring='accuracy')
print("\n=== XGBoost 5-Fold Cross-Validation ===")
print(f"Fold Accuracies: {cv_scores_xgb}")
print(f"Mean CV Accuracy: {cv_scores_xgb.mean():.4f}")
print(f"CV Accuracy Std Dev: {cv_scores_xgb.std():.4f}")
cv_results_xgb = pd.DataFrame({
    "Fold": list(range(1,6)),
    "Accuracy": cv_scores_xgb
})
cv_results_xgb.to_csv("cv_results_xgb.csv", index=False)
...
# ====================================================
# 9. SAVE MODELS & ARTIFACTS FOR REPLICATION
# ====================================================
print("\nSaving trained models and necessary artifacts for replication...")
