...
def objective(trial):
    ...
    epochs = 30  # Use fewer epochs during hyperparameter optimization

    clf = PyTorchNNClassifierWithVal(hidden_size=hidden_size,
...
study.optimize(objective, n_trials=10)  # Increase n_trials as needed for a thorough search
best_trial = study.best_trial
log_message(f"Best CV Accuracy: {best_trial.value:.4f}")
log_message(f"Best hyperparameters: {best_trial.params}")
...
best_pipeline = Pipeline([
    ('scaler', StandardScaler()),
    ('nn', PyTorchNNClassifierWithVal(hidden_size=best_hyperparams["hidden_size"],
...
# ---------------------------------------------------------
# 7.1. 5-FOLD CROSS-VALIDATION WITH BEST HYPERPARAMS (TRACKING LOSSES & ACCURACY)
# ---------------------------------------------------------
log_message("Performing 5-fold CV with the best hyperparameters for uncertainty estimates...")
kf = KFold(n_splits=5, shuffle=True, random_state=42)
fold_train_losses = []
fold_val_losses = []
fold_val_accuracies = []  # To store validation accuracy per fold
# Get classifier parameters from the best_pipeline's 'nn' step
