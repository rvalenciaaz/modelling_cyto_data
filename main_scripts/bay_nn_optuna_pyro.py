...
# Create a folder to save replication files
OUTPUT_FOLDER = "replication_files"
os.makedirs(OUTPUT_FOLDER, exist_ok=True)

# ---------------------------------------------------------
# 1. READING CSV FILES (POLARS) & COMBINING
# ---------------------------------------------------------
...
# Remove 'species' from Label (e.g., "species1" => "1")
...
# ---------------------------------------------------------
log_message("Filtering numeric features based on Median Absolute Deviation (MAD)...")
...
# ---------------------------------------------------------
# 4. BAYESIAN NEURAL NETWORK MODEL DEFINITION
# ---------------------------------------------------------
def bayesian_nn_model(x, y=None, hidden_size=32, num_layers=2, output_dim=2):
    """
    A Bayesian neural network with a configurable number of hidden layers.
    We define a standard Normal prior for each weight and bias.
    """
    input_dim = x.shape[1]
    hidden = x
...
# Utility for standard training with SVI
def train_pyro_model(
    ...
    """
    Trains a Bayesian NN with Pyro, returning (guide, list_of_train_losses).
    (No validation loss is tracked here.)
    """
    ...
    for epoch in range(num_epochs):
        ...
        if verbose and epoch % 100 == 0:
            log_message(f"[Epoch {epoch}] Training loss: {loss:.4f}")

    return guide, train_losses

# Utility for training & computing train/val loss each epoch (useful for CV)
def train_pyro_model_with_val(
    ...
    """
    Similar to `train_pyro_model`, but also calculates validation loss each epoch
    and returns (guide, train_losses, val_losses).
    """
    pyro.clear_param_store()
    guide = autoguide.AutoNormal(bayesian_nn_model)
    optimizer = pyro_optim.Adam({"lr": learning_rate})
...
def predict_pyro_model(
    ...
    """
    Uses the posterior samples (via guide) to generate predictions.
    Returns a numpy array of predicted classes (majority vote).
    """
    ...
    preds = samples["obs"].cpu().numpy()  # shape: (num_samples, X_tensor.shape[0])

    # Majority vote across samples for each instance
    final_preds = np.apply_along_axis(
        lambda row: np.bincount(row, minlength=output_dim).argmax(),
        axis=0,
        arr=preds
    )
    return final_preds
...
def objective(trial):
    """
    Objective function for Optuna. We sample hyperparameters, train the model on
    a partial train split, then validate on a small validation split,
...
log_message("Optuna hyperparameter optimization completed.")

best_params = study.best_params
best_hidden_size = best_params["hidden_size"]
best_num_layers  = best_params["num_layers"]
best_learning_rate = best_params["learning_rate"]
log_message(f"Best hyperparameters found by Optuna: {best_params}")

# ---------------------------------------------------------
# 6. 5-FOLD CROSS VALIDATION WITH PER-EPOCH LOSS TRACKING
# ---------------------------------------------------------
log_message("Starting 5-fold cross-validation with best hyperparameters and loss tracking...")

skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)

num_epochs_cv = 2000  # Example: track each epoch for CV; might want fewer for performance
fold_train_losses = []
fold_val_losses   = []
fold_accuracies  = []
...
for fold_idx, (train_index, val_index) in enumerate(skf.split(X_train, y_train)):
    ...
    # Train & record per-epoch train/val loss
    ...
    fold_acc = accuracy_score(y_fold_val, val_preds_fold)
...
log_message(f"5-fold CV accuracy (mean ± std): {cv_mean_accuracy:.4f} ± {cv_std_accuracy:.4f}")

# Save fold losses for replication
with open(os.path.join(OUTPUT_FOLDER, "cv_fold_losses.pkl"), "wb") as f:
    pickle.dump({"train_losses": fold_train_losses,
                 "val_losses":   fold_val_losses}, f)

# ---------------------------------------------------------
# 6a. PLOT PER-FOLD TRAIN/VAL LOSSES
# ---------------------------------------------------------
fold_count = len(fold_train_losses)
for i in range(fold_count):
    epochs = range(1, len(fold_train_losses[i]) + 1)
    plt.figure(figsize=(8, 5))
    plt.plot(epochs, fold_train_losses[i], label=f"Train Loss (Fold {i+1})")
    plt.plot(epochs, fold_val_losses[i],   label=f"Val Loss (Fold {i+1})")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title(f"Train vs. Val Loss (Fold {i+1})")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_FOLDER, f"fold_{i+1}_losses.png"))
    plt.close()

# ---------------------------------------------------------
# 6b. AGGREGATED PLOT WITH CONFIDENCE BANDS
# ---------------------------------------------------------
#  - We assume each fold ran for the same number of epochs.
all_train_losses = np.array(fold_train_losses)  # shape = (folds, epochs)
all_val_losses   = np.array(fold_val_losses)    # shape = (folds, epochs)
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("Aggregated 5-Fold CV Loss with Mean ± Std")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_FOLDER, "cv_aggregated_loss.png"))
plt.close()

# ---------------------------------------------------------
# 7. FINAL TRAINING ON ENTIRE TRAINING SET & TEST EVALUATION
# ---------------------------------------------------------
log_message("Retraining final model on the entire training set with best hyperparams...")

num_epochs_final = 5000  # number of epochs for final training
...
plt.figure(figsize=(10, 6))
plt.plot(epochs, final_train_losses, label='Final Training Loss')
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("Final Model (Full Training Set) Loss vs. Epoch")
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(OUTPUT_FOLDER, "final_training_loss.png"))
plt.close()

# Save final training losses for replication
with open(os.path.join(OUTPUT_FOLDER, "final_losses.pkl"), "wb") as f:
    pickle.dump(final_train_losses, f)

# Predict on the test set
log_message("Evaluating final model on the test set...")
final_test_preds = predict_pyro_model(
...
test_accuracy = accuracy_score(y_test_t, final_test_preds)
class_report = classification_report(y_test_t, final_test_preds, target_names=label_encoder.classes_)
log_message(f"Final Bayesian NN Accuracy on Test Set: {test_accuracy:.4f}")
log_message("\nClassification Report:")
log_message(class_report)

# ---------------------------------------------------------
# 8. SAVE METRICS, PARAMETERS, AND LOGS
# ---------------------------------------------------------
metrics_dict = {
...
with open(os.path.join(OUTPUT_FOLDER, "metrics_pyro.json"), "w") as f:
    json.dump(metrics_dict, f, indent=2)

# Save final guide parameters
guide_state = final_guide.state_dict()
with open(os.path.join(OUTPUT_FOLDER, "bayesian_nn_pyro_params.pkl"), "wb") as f:
...
# Save the list of kept features
with open(os.path.join(OUTPUT_FOLDER, "features_to_keep.json"), "w") as f:
    json.dump(features_to_keep, f, indent=2)

# Save the log of steps
with open(os.path.join(OUTPUT_FOLDER, "log_steps_pyro.json"), "w") as f:
    json.dump(log_steps, f, indent=2)

log_message("All done! Training + 5-fold CV + aggregated plots + final testing complete.")
